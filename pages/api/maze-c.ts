...
function renderMazeCell(x: number, y: number, cellType: Type): string {
  ...
  switch (cellType) {
    case Type.Wall:
      return `<rect x="${MARGIN.x + CELL_SIZE.x * x}" y="${
        MARGIN.y + CELL_SIZE.y * y
      }" width="${CELL_SIZE.x}" height="${
        CELL_SIZE.y
      }" fill="${WALL_COLOR}" />`;
    ...
    case Type.Passage:
      return `<rect id="${ID_PREFIX}${x}_${y}" x="${
        MARGIN.x + CELL_SIZE.x * x + PADDING.x
      }" y="${MARGIN.y + CELL_SIZE.y * y + PADDING.y}" width="${
        CELL_SIZE.x - PADDING.x * 2
      }" height="${
        CELL_SIZE.y - PADDING.y * 2
      }" fill="${SOLUTION_COLOR}" opacity="${opacity}" />`;
  }
...
export default async function handler(
  ...
  const importObject = {
    env: {
      ...
      render_move_in: (x, y) => {
        if (ANIMATE) {
          svgSolution.push(
            `<animate xlink:href="#${ID_PREFIX}${x}_${y}" attributeName="opacity" from="0" to="${WALK_OPACITY}" dur="${ANIMATION_DELAY}s" begin="${tick}s" fill="freeze"/>`
          );
      ...
      render_move_out: (x, y) => {
        if (ANIMATE) {
          svgSolution.push(
            `<animate xlink:href="#${ID_PREFIX}${x}_${y}" attributeName="opacity" from="${WALK_OPACITY}" to="0" dur="${ANIMATION_DELAY}s" begin="${tick}s" fill="freeze"/>`
          );
          tick += ANIMATION_DELAY;
        }
      },
      render_solution: (x, y) => {
        if (ANIMATE || ANIMATE_FINAL) {
          svgSolution.push(
            `<animate xlink:href="#${ID_PREFIX}${x}_${y}" attributeName="opacity" from="${WALK_OPACITY}" to="${SOLUTION_OPACITY}" dur="${ANIMATION_DELAY}s" begin="${tick}s" fill="freeze"/>`
          );
          tick += ANIMATION_DELAY;
        }
      },
  ...
  console.log(await exports.test_fn());
